...
CONTAINER=$(sudo podman ps | grep homebridge | cut -d " " -f1)
...
CONFIG=$(sudo podman exec "$CONTAINER" cat /homebridge/config.json)
NAME=$(echo "$CONFIG" | jq -r .bridge.name)
MAC=$(echo "$CONFIG" | jq -r .bridge.username)
PORT=$(echo "$CONFIG" | jq -r .bridge.port)

ACCESSORY_CONFIG=$(sudo podman exec "$CONTAINER" cat /homebridge/persist/AccessoryInfo.${MAC//:/}.json)
...
ACCESSORY_FILES=$(sudo podman exec "$CONTAINER" find persist/ -name 'AccessoryInfo.*.json' | grep -v ${MAC//:/})
...
do
  ...
  ACCESSORY_CONFIG=$(sudo podman exec "$CONTAINER" cat /homebridge/$i)
  SETUPID=$(echo "$ACCESSORY_CONFIG" | jq -r .setupID)
  CATEGORY=$(echo "$ACCESSORY_CONFIG" | jq -r .category) 
  NAME=$(echo "$ACCESSORY_CONFIG" | jq -r .displayName)
  MAC=$(echo "$i" | cut -d. -f2 | sed 's/\(..\)/\1:/g;s/:$//')
  PORT=$(sudo podman logs "$CONTAINER" | grep "${NAME} is running on port" | tail -n 1 | awk -F ' ' '{print $NF}' | cut -d. -f1)
